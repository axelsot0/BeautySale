        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-pink via-lavender to-butter py-14 md:py-20">
          ...
          <div className="relative max-w-7xl mx-auto px-4 md:px-8 text-cream">
            ...
            <p className="text-lg md:text-xl text-cream/90 mt-3 max-w-xl">
              {products.length > 0
                ? fmt(dict.sale_count, { n: products.length, m: maxDiscount })
                : dict.sale_soon}
            </p>

            {products.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-6">
                <Stat label={dict.stat_products} value={products.length} />
                <Stat label={dict.stat_avg} value={`${avgDiscount}%`} />
                <Stat label={dict.stat_upto} value={`-${maxDiscount}%`} accent />
              </div>
            )}
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-10">
          {products.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-plum-soft">
              ...
              <p className="font-display text-2xl">{dict.sale_empty_title}</p>
              <p className="text-sm">{dict.sale_empty_sub}</p>
              <a
                href="/productos"
